...
export default function PriceRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-[#1A1A1A] font-mono text-sm">
      <span className="min-w-0 flex-1 truncate">
        {href ? (
          <Link href={href} className="text-[#F5F5F5] hover:text-[#00FF94] transition-colors">
            {label}
          </Link>
        ) : (
      ...
      {sublabel && (
        <span className="text-[#555555] hidden sm:block shrink-0">{sublabel}</span>
      )}

      <span className="text-[#555555] hidden md:block shrink-0 w-20 text-right">
        {quote?.successRate ? `${quote.successRate}%` : "—"}
      </span>

      <span className="shrink-0 w-20 text-right">
        {quote ? (
          <span className="text-[#00FF94]">{formatPrice(quote.price)}</span>
        ) : (
          // A failed lookup must read as "not right now", never as free.
          <span className="text-[#555555]">n/a</span>
        )}
      </span>
    </div>
  );
...
export function PriceTableHeader({ first, second }: { first: string; second?: string }) {
  return (
    <div className="flex items-center justify-between gap-4 pb-2 border-b border-[#1A1A1A] font-mono text-xs text-[#555555] uppercase tracking-wider">
