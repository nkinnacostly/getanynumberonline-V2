...
export default function OrderForm({
  ...
  // Smallest allowed top-up (min $5) that covers the shortfall.
  const fundAmount = Math.min(
    500,
    Math.max(5, Math.ceil((price ?? 0) - balance)),
  );
  const startFunding = () =>
    topup.open(fundAmount, { onFunded: () => onFunded?.() });
          ...
          >
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={
                  topup.opening || topup.scriptError || !topup.available
                }
